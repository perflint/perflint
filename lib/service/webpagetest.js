...
var wpt = {
  ...
  getResults: function(config, cb) {
    var url = config.URL || config.test,
        test = config.test,
        key = config.key,
        timeout = config.timeout,
        requestTest = new WebPageTest(config.server, key),
      ...
      requestTest.runTest(url,
        ...
        if (err) {
          ...
          if (err.error && err.error.code === 'TIMEOUT') {
            ...
            var isWptServer = config.server !== 'www.webpagetest.org' ? 'Please check server is a valid WebPageTest server' : ''
            log.error('Error: Test request failed - ' + isWptServer)
          }
  ...
  printInfo: function(config, results) {
