...
const wpt = {
  validateResults(err, data, config) {
    ...
    /* istanbul ignore if */
    if (config.info) {
      this.printInfo(config, data.data)
    }

    const parsed = this.translateResults(config, data.data)
    return parsed
  },
  ...
  translateResults(config, results) {
    ...
    return {
      ...
      requestsHTML:
        filtered.breakdown && filtered.breakdown.html
          ? filtered.breakdown.html
          : '',
      requestsJS:
        filtered.breakdown && filtered.breakdown.js
          ? filtered.breakdown.js
          : '',
      requestsCSS:
        filtered.breakdown && filtered.breakdown.css
          ? filtered.breakdown.css
          : '',
      requestsImage:
        filtered.breakdown && filtered.breakdown.image
          ? filtered.breakdown.image
          : '',
      requestsFlash:
        filtered.breakdown && filtered.breakdown.flash
          ? filtered.breakdown.flash
          : '',
      requestsFont:
        filtered.breakdown && filtered.breakdown.font
          ? filtered.breakdown.font
          : '',
      requestsOther:
        filtered.breakdown && filtered.breakdown.other
          ? filtered.breakdown.other
          : '',
      ...
      score_progressive_jpeg: filtered.score_progressive_jpeg
        ? filtered.score_progressive_jpeg
        : '',
