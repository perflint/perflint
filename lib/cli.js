...
var command = {
  ...
  printResults: function(results, format) {
    ...
    if (!formatter) {
      cli.error('Could not find formatter: ' + format)
      return false
    }
    formatter(results)
  },
  ...
  interpret: function(args) {
    ...
    if (!opts.key) {
      cli.error('An API key for WebPageTest must be specified.')
      return 1
    }
  ...
  run: function(opts, config) {
    var results = perflint.lint(opts, config)
    if (!command.printResults(results, opts.format)) {
      return false
    }
  }
}
